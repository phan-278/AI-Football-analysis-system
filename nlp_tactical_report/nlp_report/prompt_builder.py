"""
nlp_report/prompt_builder.py
────────────────────────────
Nhận match_stats dict (output của StatsAggregator)
→ Build system prompt + user prompt chuẩn cho LLM
→ LLM trả về JSON báo cáo chiến thuật tiếng Việt
"""
...
Trả lời DUY NHẤT dưới dạng JSON hợp lệ với cấu trúc sau (không thêm markdown, không giải thích):
{
  ...
  "conclusion": "Kết luận tổng quan 3-4 câu về chiến thuật và diễn biến trận đấu"
    ...
    def _build_user_prompt(self) -> str:
        ...
        for label, team, tid in [("ĐỘI 1", t1, 1), ("ĐỘI 2", t2, 2)]:
    ...
    def _format_team(self, label: str, team: dict) -> list[str]:
        ...
        lines = [
            ...
            f"  Lịch sử sơ đồ       : {' → '.join(team.get('formation_history', ['N/A']))}",
