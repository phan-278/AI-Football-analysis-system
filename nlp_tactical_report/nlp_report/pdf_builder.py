"""
nlp_report/pdf_builder.py
─────────────────────────
Nhận report dict (output của LLMClient.generate)
+ match_stats dict (output của StatsAggregator)
→ Xuất file PDF báo cáo chiến thuật chuyên nghiệp bằng ReportLab

Layout PDF:
  ① Header: tiêu đề + thông tin trận đấu
  ② Tổng quan trận đấu
  ③ Stat cards so sánh nhanh 2 đội
  ④ Phân tích Đội 1 (formation, tactical summary, strengths/weaknesses)
  ⑤ Phân tích Đội 2 (tương tự)
  ⑥ Bảng so sánh số liệu chi tiết
  ⑦ Phân tích so sánh (possession, pressing, space, physical, key_difference)
  ⑧ Cầu thủ nổi bật
  ⑨ Kết luận
  ⑩ Footer

Usage:
    pdf = PDFBuilder(report, match_stats, team1_name="Team A", team2_name="Team B")
    pdf.save("outputs/tactical_report.pdf")
"""
