...
class CameraMovementEstimator():
        ...
        for object, object_tracks in tracks.items():
            for frame_num, track in enumerate(object_tracks):
                for track_id,track_info in track.items():
                   ...
                   position_adjust = (position[0]-camera_movement[0],position[1]-camera_movement[1])
                   tracks[object][frame_num][track_id]['position_adjusted'] = position_adjust
    ...
    def get_camera_movement(self,frames,read_from_stub = False,stub_path = None):
        ...
        old_feature = cv2.goodFeaturesToTrack(old_gray,**self.features)

        for frame_num in range(1,len(frames)):
            new_gray = cv2.cvtColor(frames[frame_num],cv2.COLOR_BGR2GRAY)
            new_feature, _ , _ = cv2.calcOpticalFlowPyrLK(old_gray,new_gray,old_feature,None,**self.lk_params)   
            ...
            if max_distance > self.min_distance:
                ...
                old_feature = cv2.goodFeaturesToTrack(new_gray,**self.features)

            old_gray = new_gray.copy()

        # Save camera movement
